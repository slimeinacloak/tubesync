...
class Source(models.Model):
    ...
    SOURCE_RESOLUTIONS = (
        SOURCE_RESOLUTION_360P,
        SOURCE_RESOLUTION_480P,
        SOURCE_RESOLUTION_720P,
        SOURCE_RESOLUTION_1080P,
        SOURCE_RESOLUTION_1440P,
        SOURCE_RESOLUTION_2160P,
        SOURCE_RESOLUTION_4320P,
    ...
    enable_sponsorblock = models.BooleanField(
        _("enable sponsorblock"), default=True, help_text=_("Use SponsorBlock?")
    )
    ...
    has_failed = models.BooleanField(
        _("has failed"), default=False, help_text=_("Source has failed to index media")
    )

    write_subtitles = models.BooleanField(
        _("write subtitles"), default=False, help_text=_("Download video subtitles")
    )
    ...
    def get_image_url(self):
        if self.source_type == self.SOURCE_TYPE_YOUTUBE_PLAYLIST:
            raise SuspiciousOperation(
                "This source is a playlist so it doesn't have thumbnail."
            )
...
class Media(models.Model):
    ...
    def download_media(self):
        ...
        download_youtube_media(
            ...
            self.source.sponsorblock_categories.selected_choices,
    ...
    def calculate_episode_number(self):
        if self.source.source_type == Source.SOURCE_TYPE_YOUTUBE_PLAYLIST:
            ...
            filtered_media = Media.objects.filter(
                source=self.source, published__year=self_year
            )
...
class MediaServer(models.Model):
    ...
    SERVER_TYPE_CHOICES = ((SERVER_TYPE_PLEX, _("Plex")),)
    ICONS = {
    ...
    port = models.PositiveIntegerField(
        _("port"), db_index=True, help_text=_("Port number of the media server")
    )
